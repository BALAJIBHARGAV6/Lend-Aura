module aura_lend::registry {
    ...
    struct UserReputation has key, store, drop {
    ...
    public fun record_loan_funded(lender: address, borrower: address, amount: u64) acquires GlobalRegistry {
        ...
        if (simple_map::contains_key(&registry.user_reputations, &lender)) {
            ...
            let lender_rep = get_or_create_user_reputation(lender);
            let updated_rep = UserReputation {
                user_address: lender_rep.user_address,
                total_borrowed: lender_rep.total_borrowed,
                total_lent: amount,
                total_repaid: lender_rep.total_repaid,
                loans_taken: lender_rep.loans_taken,
                loans_given: 1,
                successful_repayments: lender_rep.successful_repayments,
                defaulted_loans: lender_rep.defaulted_loans,
                reputation_score: lender_rep.reputation_score + 5,
                joined_at: lender_rep.joined_at,
                last_activity: timestamp::now_seconds(),
            };
            simple_map::upsert(&mut registry.user_reputations, lender, updated_rep);
        };
        ...
        if (simple_map::contains_key(&registry.user_reputations, &borrower)) {
            ...
            let borrower_rep = get_or_create_user_reputation(borrower);
            let updated_rep = UserReputation {
                user_address: borrower_rep.user_address,
                total_borrowed: amount,
                total_lent: borrower_rep.total_lent,
                total_repaid: borrower_rep.total_repaid,
                loans_taken: 1,
                loans_given: borrower_rep.loans_given,
                successful_repayments: borrower_rep.successful_repayments,
                defaulted_loans: borrower_rep.defaulted_loans,
                reputation_score: borrower_rep.reputation_score,
                joined_at: borrower_rep.joined_at,
                last_activity: timestamp::now_seconds(),
            };
            simple_map::upsert(&mut registry.user_reputations, borrower, updated_rep);
        };
